<!-- SPDX-License-Identifier: MulanOWL-PL OR CC0-1.0 -->

# k4m7v2pz 的博客

**开源独立游戏资产 & 工具** — 可拉取、可修改、可商用的代码与资产

---

### 🔧 Windows / 工具

- [rust-verb-shell](https://atomgit.com/k4m7v2pz/rust-verb-shell) — 自研跨平台结构化 Shell（rvs），替换 PowerShell/Nushell，部分替代 zsh/bash（Rust）
- [xtl-toolchain](https://atomgit.com/k4m7v2pz/xtl-toolchain) — 玄铁语言工具链封装（Rust）
- [rust-cafe-tune](https://atomgit.com/k4m7v2pz/rust-cafe-tune) — 网吧上机客制化 CLI：鼠标优化、软件安装、壁纸与日志管理（Rust，跨平台，常用常维护）
- [rust-backup-tool](https://atomgit.com/k4m7v2pz/rust-backup-tool) — 跨操作系统备份程序（Rust，自用并持续维护）
- [rust-shorts-lab](https://atomgit.com/k4m7v2pz/rust-shorts-lab) — 代码生成视频，告别剪辑软件（Rust）
- [atomcode-skills-personal](https://atomgit.com/k4m7v2pz/atomcode-skills-personal) — 个人 AtomCode Skills 备份，工作流开源少绕路

[▶ 更多](https://atomgit.com/k4m7v2pz)

### 🎮 游戏项目

- [rust-bevy-mighty-rodent](https://atomgit.com/k4m7v2pz/rust-bevy-mighty-rodent) — Mighty Rodent 逆向重写（Rust/Bevy，热衷维护中）
- [rust-bevy-endless-corridor](https://github.com/k4m7v2pz/rust-bevy-endless-corridor) — 开发主阵地：Agent 友好的 2 空格缩进代码，让恐怖/叙事类游戏可通过 Agent 开发（Rust/Bevy）
- [rust-bevy-narrative-path-craft](https://github.com/k4m7v2pz/rust-bevy-narrative-path-craft) — 剧情引擎占位：Agent/LLM 友好的剧情数据结构，LLM 能理解「梗」与角色台词动机（Rust/Bevy）
- [rust-egui-chplus-ide](https://github.com/k4m7v2pz/rust-egui-chplus-ide) — egui 版 IDE（Rust）
- [rust-tauri-chplus-ide](https://github.com/k4m7v2pz/rust-tauri-chplus-ide) — Tauri 版 IDE（Svelte）
- [GachimuchiSampling](https://github.com/k4m7v2pz/GachimuchiSampling) — 哲♂学采样（PowerShell）
- [rust-bevy-sky-rebound](https://atomgit.com/k4m7v2pz/rust-bevy-sky-rebound) — 弹跳至天空游戏（Rust/Bevy，热衷维护）
- [rust-bevy-bomber-man](https://atomgit.com/k4m7v2pz/rust-bevy-bomber-man) — 炸弹人游戏（Rust/Bevy，喜欢玩）
- [rust-bevy-open-bedwars](https://atomgit.com/k4m7v2pz/rust-bevy-open-bedwars) — 空岛战争（Rust/Bevy，开发中）
- [rust-bevy-infinite-oilwell](https://atomgit.com/k4m7v2pz/rust-bevy-infinite-oilwell) — 类红警2 的无限油井（Rust/Bevy）
- [ts-oyrw-infinite-oilwell](https://atomgit.com/k4m7v2pz/ts-oyrw-infinite-oilwell) — 无限油井 TS 版（TypeScript）
- [rust-bevy-omniscent](https://atomgit.com/k4m7v2pz/rust-bevy-omniscent) — DOS 演示程序 Omniscent 重写（Rust/Bevy）
- [rust-bevy-cruise-evade](https://atomgit.com/k4m7v2pz/rust-bevy-cruise-evade) — 极品飞车 9 感觉的追逃玩法（Rust/Bevy，起步中）

### 📓 其他项目

- [rust-yumo](https://github.com/k4m7v2pz/rust-yumo) — yumo 音频播放库（Rust）
- [python-arcade-endless-corridor](https://github.com/k4m7v2pz/python_arcade_endless_corridor) — Python Arcade 版无尽走廊（Python）
- [gitcode-knowledge](https://atomgit.com/k4m7v2pz/gitcode_knowledge) — CSDN 文章 Markdown 备份仓（厌倦 CSDN 网页体验可看这里或本站）
- [rolling-ndjson-log](https://atomgit.com/k4m7v2pz/rolling-ndjson-log) — 滚动 NDJSON 日志工具（Rust，agent 推荐）
- [hot-reload-rules](https://atomgit.com/k4m7v2pz/hot-reload-rules) — 热重载规则工具（Rust，agent 推荐）
- [tokio-ws-multi-client](https://atomgit.com/k4m7v2pz/tokio-ws-multi-client) — Tokio WebSocket 多客户端（Rust，agent 推荐）
- [retro-chat](https://atomgit.com/k4m7v2pz/retro-chat) — 复古 Web 前端，复刻 ChatGPT 3.5 时代网页风格
- [rust-game-chat](https://atomgit.com/k4m7v2pz/rust-game-chat) — 为 RTS 游戏准备的聊天系统（Rust）
- [xonotic-config](https://atomgit.com/k4m7v2pz/xonotic-config) — Xonotic 游戏配置文档示例

[▶ 全部仓库](https://atomgit.com/k4m7v2pz)

---

### 📓 Blog / Notes

操作系统、自动化、部署相关的技术笔记

[▶ 阅读博客](SUMMARY.md)

---

[▶ 关于作者](about.md)
