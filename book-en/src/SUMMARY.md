# Summary

[Home](index.md)
[About](about.md)

---

# Operating System

## GNU/Linux
- [Install Google Pinyin on Ubuntu Derivatives](blog/operating-system/gnulinux/google-pinyin-installation-ubuntu-derivatives.md)
- [DD Install Debian 12 on a Cheap VPS Using CentOS 7](blog/operating-system/gnulinux/centos7-jumpbox-dd-debian12-vps.md)
- [Simulating macOS Keyboard on Arch Linux](blog/operating-system/gnulinux/archlinux-macos-keyboard-keyd-sway-wezterm.md)
- [Minimal Arch Linux Installation Guide](blog/operating-system/gnulinux/minimal-archlinux-install-guide.md)
- [XFCE Desktop + lightdm Package List Reference](blog/operating-system/gnulinux/xfce-lightdm-package-list.md)
- [ThinkPad E490 Fan/Temperature Auto-Control Log: thinkfan Absent, a Hand-Written Curve Script Takes Over](blog/operating-system/gnulinux/thinkpad-e490-fan-control.md)

## Windows
- [Embedded Python pip Installation](blog/operating-system/windows/embed-python-pip-installation-windows.md)
- [7-Zip SFX Standard Stub Selection](blog/operating-system/windows/7zip-sfx-stub-selection.md)
- [Fix RunProgram Failure](blog/operating-system/windows/fix-7zip-sfx-runprogram-failure.md)
- [LLM Silent EXE Deployment](blog/operating-system/windows/llm-deploy-silent-7zip-sfx.md)
- [Set Zed as bat Editor](blog/operating-system/windows/set-zed-as-bat-editor.md)
- [Windows Terminal Ultimate Overhaul](blog/operating-system/windows/windows-terminal-overhaul.md)
- [Remote Management of Windows 10 LTSB in Restricted Networks](blog/operating-system/windows/windows-ltsb-winrm-remote-management.md)
- [Install Nushell via Scoop on Windows](blog/operating-system/windows/scoop-install-nushell-windows.md)
- [System Management Scripts Archive](blog/operating-system/windows/system-management-scripts.md)
- [PowerShell Invoke-RestMethod SSL/TLS Secure Channel Errors: Troubleshooting](blog/operating-system/windows/powershell-invoke-restmethod-ssl-tls.md)
- [Windows VPS Terminal Display Completely Garbled: Investigating and Fixing Three Stacked Independent Problems](blog/operating-system/windows/windows-vps-terminal-garbled.md)
- [Fixing SSH-into-Windows Character-by-Character Output Accumulation: \r Carriage Return Treated as Newline, winpty with Win10 1607 and No ConPTY](blog/operating-system/windows/ssh-windows-output-buffering.md)

# Tools
- [Nushell + AtomCode Configuration Pitfalls](blog/tools/nushell-atomcode-config-pitfalls.md)
- [Five Deep Pitfalls in Extracting a 40GB Multi-Volume Encrypted Firmware Image on macOS](blog/tools/macos-multivolume-zip-chinese-password.md)
- [CSDN Writing Agent Content Review: What Gets Rejected and How to Get Around It](blog/tools/csdn-writing-agent-content-review.md)
- [WezTerm + Nushell + Cargo Not Found](blog/tools/wezterm-nushell-cargo-not-found.md)
- [CSDN Writing Agent Input Limits](blog/tools/csdn-writing-agent-input-limits.md)
- [macOS Nushell Config Path Pitfall](blog/tools/macos-nushell-config-path-pitfall.md)
- [NuShell AI Collaboration Pitfalls](blog/tools/nushell-ai-collaboration-pitfalls.md)
- [From PowerShell to Nushell Migration](blog/tools/powershell-to-nushell-migration.md)
- [rvs (rust-verb-shell): A Structured Shell for Humans and AI Agents](blog/tools/rvs-structured-shell-intro.md)
- [rvs (rust-verb-shell) Changelog Overview 26.7.17 – 26.8.10](blog/tools/rvs-changelog-26.7.17-26.8.10.md)
- [rvs (rust-verb-shell) Changelog Overview 26.7.12 – 26.7.17](blog/tools/rvs-changelog-26.7.12-26.7.17.md)
- [rvs (rust-verb-shell) Iteration Overview 26.7.0 – 26.7.12](blog/tools/rvs-iteration-26.7.0-26.7.12.md)
- [WezTerm CJK Glyph Rendering Chaos: Diagnosis and Fix](blog/tools/wezterm-cjk-glyph-fix.md)
- [macOS Automated Circuit-Design Validation Workflow: KiCad CLI + ngspice + Python, Built From Scratch](blog/tools/macos-kicad-ngspice-circuit-validation.md)

# Programming
- [Rust Bevy 0.13 to 0.14 Upgrade Guide](blog/programming/rust-bevy-0.13-to-0.14-upgrade-guide.md)
- [RK3326 Device Tree Panel Debug: LCD Backlight On but Black Screen After Kernel Upgrade](blog/programming/rk3326-dtb-panel-debug.md)
- [XuanTie 0.17.5 InputExpression Bug](blog/programming/xuantie-input-expression-bug.md)
- [Bevy 0.14 Cursor Pitfalls](blog/programming/bevy-0.14-cursor-pitfalls.md)
- [Bevy 0.14 FPS Camera Pitfalls](blog/programming/bevy-0.14-fps-camera-pitfalls.md)
- [AtomCode Spinner Labels Explained](blog/programming/atomcode-spinner-thinking-labels.md)
- [AtomCode Spinner Wordlist Errata: 85% Was Fabricated](blog/programming/atomcode-spinner-wordlist-errata.md)
- [From AtomCode to Ascend NPU](blog/programming/atomcode-ascend-npu-free-tier.md)
- [AtomCode Project: Agent Memorized-Writing SOP](blog/programming/atomcode-writing-sop.md)
- [Inside AtomCode's Ctrl+O: The Evolution from Tool Output to Reasoning Visibility](blog/programming/atomcode-ctrl-o-evolution.md)
- [Tracing the AtomCode fmt_dur Controversy](blog/programming/atomcode-fmt-dur-controversy.md)
- [Running LLMs Locally on a 16GB Mac: A Practical Guide to ollama's LAN OpenAI-Compatible API](blog/programming/ollama-lan-openai-api-mac.md)
- [Bevy 0.14.2 Player Sprite Not Rendering: Full Debugging Log](blog/programming/bevy-0.14.2-sprite-not-rendering.md)
- [Bytebeat Moog City Reconstruction v3.2: Technical Analysis and Implementation](blog/programming/bytebeat-moog-city-reconstruction.md)
- [From rg to rr: The r-Prefix Fetish of CLI Tools](blog/programming/rg-to-rr-r-prefix-cult.md)
- [Bevy 0.14 Black Window Troubleshooting: Camera2d Is Just a Marker Component, spawn Camera2d Won't Create a Camera](blog/programming/bevy-0.14-black-window-camera2d.md)
- [Rust Cross-Compilation in Practice: From Mac to Linux / Windows (Using rvs as an Example)](blog/programming/rust-cross-compile-mac-linux-windows.md)
- [egui CJK Rendering Sits Too High: A FontTweak Baseline Alignment Fix Log](blog/programming/egui-cjk-rendering-fonttweak.md)
- [Fixing Terminal Table Line Wrapping: crossterm Returns the Buffer Width Instead of the Window Width (dwSize vs srWindow)](blog/programming/crossterm-terminal-table-wrap.md)

# Games
- [Debugging Black Screen on Mighty Rodent Splash Screen](blog/games/mighty-rodent-splash-black-screen-debug.md)
- [R36S System Card and Game Card Separation: EmuELEC Dual-Card Setup](blog/games/r36s-emuelec-dual-card-separation.md)
- [Game Config for AI Agents: JSON Schema and an ai_context Annotation-Layer Design](blog/games/game-config-json-schema-ai-context.md)
- [Arch Linux + Wine + i3wm Retro Gaming Environment](blog/games/archlinux-wine-i3wm-retro-game-env.md)
- [Windowed Browser Game Resolution: CDP Outer Size vs Content Area Size](blog/games/browser-windowed-game-resolution-cdp.md)
- [Porting the Yellow Alert MOD to ChronoDivide: Pitfall Log](blog/games/yellow-alert-chronodivide-mod-port.md)
- [Porting an RA2 MOD to OpenYRWeb: BGM Mounting and UI Overflow Fixes](blog/games/openyrweb-ra2-bgm-and-ui-fixes.md)
- [OpenYRWeb Browser Engine Internals: VFS-mix Loading, Locale Detection, and the Audio Pipeline](blog/games/openyrweb-vfs-mix-locale-audio.md)
- [Injecting Game Data into the Browser via OPFS](blog/games/openyrweb-opfs-game-data-injection.md)
- [R36S Handheld Game Migration Field Notes: EmuELEC Dual-Card Cleanup](blog/games/r36s-emuelec-game-migration.md)
- [R36S System Migration in Practice: Moving Losslessly from a No-Name Cheap TF Card to a Branded Card](blog/games/r36s-tf-card-migration-rk3326.md)
- [Reverse-Engineering the RA2/YR CSF File Format](blog/games/ra2-yr-csf-format-reverse.md)
- [BGM Mounting and Display-Name Translation for RA2 MODs](blog/games/ra2-mod-bgm-thememd-csf.md)
- [Pitfalls Porting an RA2 MOD to OpenYRWeb](blog/games/ra2-mod-openyrweb-port-pitfalls.md)
- [Bevy wgpu R36S Handheld Screen-on Conclusion](blog/games/bevy-wgpu-r36s-screen.md)
- [Bevy 0.14 Rendering Full Pipeline on the R36S](blog/games/bevy-0.14-r36s-rendering.md)
- [Porting a Bevy 0.14 Game to the R36S Handheld](blog/games/bevy-0.14-port-to-r36s.md)
- [OpenYRWeb: Troubleshooting Battlefield Cursor Speed Anomalies](blog/games/openyrweb-cursor-speed.md)
- [OpenYRWeb Engine Configuration Migration: A Practical Guide from INI to JSON Schema](blog/games/openyrweb-config-migration-ini-json.md)
- [OpenYRWeb: Complete Solution for Displaying Simplified Chinese Interface When Loading a Traditional Chinese MOD](blog/games/openyrweb-traditional-chinese-mod.md)

# Suno AI
- [Red Alert 2 Yellow Alert Main Menu Music — Suno AI Style Prompt](blog/sunoai/yellow-alert-suno-style-prompt.md)
